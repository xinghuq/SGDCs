...
multRegD_env=function(g,env,ncode,nperm){
  ...
  mrdiff_ev=setNames(as.list(c(1:ncol(env))),c(colnames(env)))
  mrForm=list()
  for (i in seq_along(evdis)) {
    mrForm[[i]]=list()
    mrForm[[i]]<- as.formula(paste("DeltaDp ~", paste("evdis$",names(evdis)[i]), sep=" "))
    mrdiff_ev[[i]]=MRM(mrForm[[i]], method="linear",nperm=nperm, mrank = FALSE)
   # mrdiff_ev[[i]]=MRM(DeltaDp ~ evdis[[i]], method="linear",nperm=nperm, mrank = FALSE)
    
  }
  
  return(list(All=mrdiff_ev))
