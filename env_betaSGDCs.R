...
partcomdis_D_env=function(comm,g,env,ncode,permutations){
...
######## here we should induce permutation that test their signiciance
  ...
  for (i in seq_along(evdis)) {
    pcor[[i]]=mantel.partial(Dcom, DeltaDp, evdis[[i]], permutations, method = "pearson")
   
  }
  return(list(PairwiseDpop = DeltaDp, pairwiseDcom = Dcom, evdis=evdis, pcor=pcor))
}
