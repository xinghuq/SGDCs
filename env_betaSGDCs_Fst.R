...
PartComDis_Fst_env=function(comm,g,env,ncode,method,permutations){
  ...
  pcor=setNames(as.list(c(1:ncol(env))),c(colnames(env)))
  
  for (i in seq_along(evdis)) {
    pcor[[i]]=mantel.partial(Comdist, PFst, evdis[[i]], permutations, method = "pearson")
    
  }
  return(list(PairwiseFst = PFst, pairwiseDcom = Comdist, PcorComdis_Fst = pcor))
}
