...
ComDis_Fst_env=function(comm,g,env,ncode,method,nrepet){
  ...
  PFstevcor=setNames(as.list(c(1:ncol(env))),c(colnames(env)))
  for (i in seq_along(evdis)) {
    comevcor[[i]]=mantel.randtest(Comdist,evdis[[i]], nrepet)
    PFstevcor[[i]]=mantel.randtest(PFst, evdis[[i]], nrepet)
  }
  return(list(corComdis_env = comevcor,CorPFstenv=PFstevcor))
}
