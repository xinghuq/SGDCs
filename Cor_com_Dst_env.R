...
corcomdis_D_env=function(comm,g,env,ncode,nrepet){
  ...
  PDevcor=setNames(as.list(c(1:ncol(env))),c(colnames(env)))
  for (i in seq_along(evdis)) {
    comevcor[[i]]=mantel.randtest(Dcom,evdis[[i]], nrepet)
    PDevcor[[i]]=mantel.randtest(DeltaDp, evdis[[i]], nrepet)
  }
  
  return(list(PairwiseDpop = DeltaDp, pairwiseDcom = Dcom, evdis=evdis, corcomDis_ev=comevcor,corD_ev=PDevcor))
}
